...
def read_helm_data(helm_data_path: str, datasets_list: List[str], verbose: bool=False) -> dict:
    '''
    ...
    for dataset in datasets_list:
        ...
        for task in task_list:
            ...
            matching_results = [t_dir for t_dir in tasks_dirs if (task['name'] in t_dir and 'groups' not in t_dir)]
            if len(matching_results) == 0:
                if verbose:
                    print("task not found : %s"%task['name'])
            
            # Get the tested models names
            tested_models = [result_name.split('model=')[-1].split(',')[0] for result_name in matching_results]
            ...
            for this_result, this_model in zip(matching_results, tested_models):
                ...
                for r in task_results:
                    try:
                        if (r["name"]["name"] == task['metric'] and r["name"]["split"] == task['split']):
                            metric_val = r[task['field']]
                            
                            if task['field'] == 'bits_per_byte':
        ...
        tasks_results_dict = {k:np.mean(v) for k,v in tasks_results_dict.items() if len(v) == len(task_list)}
