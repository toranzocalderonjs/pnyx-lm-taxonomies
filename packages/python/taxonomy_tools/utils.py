...
def load_taxonomy(file_path: str, return_all: List=False, verbose: bool=False) -> any:
    '''
    ...
    with open(file_path) as f:
        for line in f:
            ...
            if "{" in line:
                ...
                # Get nodes
                from_n = line.split(" -> ")[0].strip().replace(";", "").replace(":", "---")
                to_n = line.split(" -> ")[-1].strip().replace(";", "").replace(":", "---")
    ...
    for edge in taxonomy_graph.edges:
        if (taxonomy_graph.nodes[edge[0]].get('datasets', None) == None) or \
            taxonomy_graph.nodes[edge[1]].get('datasets', None) == None:
            undefined_edges.append(edge)
        else:
            measurable_edges.append(edge)
    if verbose:
        print("%d undefined edges of %d edges (%d are potentially measurable)"%(len(undefined_edges), len(taxonomy_graph.edges), len(measurable_edges)))        
    ...
    base_nodes = [node for node, out_degree in taxonomy_graph.out_degree() if out_degree == 0]
                ...
                for dataset in dataset_list_aux:
                    if dataset in dataset_list:
                        ...
                        for node in node_path:
                            print("\t%s"%node_path)
                        raise ValueError("Detected downstream dataset sharing in node %s with %s on dataset %s"%(node,edge[0], dataset))
                # Go deeper
                recursive_explore(node_path + [edge[0]], dataset_list+dataset_list_aux)
        return 
...
def filter_for_full_samples(samples_dict: dict) -> dict:
    ...
    use_models = [model for model, count in included_dataset_count.items() if count == len(samples_dict.keys())]
    use_models.sort()
    samples_fullytested_dict = dict()
    for dataset in samples_dict.keys():
        filtered_dict = {k:v for k,v in samples_dict[dataset].items() if k in use_models}
        # Also sort
...
def get_taxonomy_datasets_metrics_dataframe(samples_dict: dict) -> pd.DataFrame:
    ...
    metrics_data_matrix = np.zeros((len(samples_dict.keys()), len(samples_dict[list(samples_dict.keys())[0]].keys())))
    ...
    # Create dataframe
    metric_data_df = pd.DataFrame(metrics_data_matrix.T, index=list(samples_dict[list(samples_dict.keys())[0]].keys()), columns=list(samples_dict.keys()))

    return metric_data_df
    
    
def get_taxonomy_datasets_node_dataframe(samples_dict: dict, taxonomy_graph : nx.classes.digraph.DiGraph, verbose: bool=False) -> pd.DataFrame:
    '''
    Receives a filtered sample dictionary (only with fully tested models) and
...
def get_taxonomy_nodes_correlation(data_df: pd.DataFrame, taxonomy_graph : nx.classes.digraph.DiGraph, verbose: bool=False) -> Tuple[np.array, np.array]:
    '''
...
def get_taxonomy_per_edge_correlation( taxonomy_graph : nx.classes.digraph.DiGraph, samples_dict: dict, verbose: bool=False) -> np.array:
    '''
    ...
    correlation_matrix_imbalanced = np.zeros((len(taxonomy_graph.nodes), len(taxonomy_graph.nodes)))
    nodes_array = np.array(taxonomy_graph.nodes())
    for edge in taxonomy_graph.edges:
        # Get adj matrix locations
        x = np.argwhere(nodes_array==edge[0])[0][0]
